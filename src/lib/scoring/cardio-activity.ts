...
export interface CardioResult {
  ...
  /**
   * What a structured interval/fartlek session was scored on, when one was.
   *
   * Null for every ordinary session, and for an interval logged without the
   * optional work/rest breakdown — that still falls back to session-average
   * scoring exactly as before, and the readout must not appear and imply
   * otherwise.
   *
   * Optional for the same reason as `ageGradeFactor` above: results persisted
   * before this field existed genuinely do not have it, and a reader must not
   * treat its absence as "there was no breakdown".
   */
