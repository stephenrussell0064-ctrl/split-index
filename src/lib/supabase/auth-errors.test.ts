...
/* ── Merged in from the app-store line ──────────────────────────────
   A different concern from the enumeration tests above: what App
   Review sees when an OAuth provider ships but is not switched on.
   Guideline 4.8 requires the Apple button wherever Google appears, so
   the button exists whether or not the provider is configured. */
